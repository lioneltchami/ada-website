...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      name: "sourceProject",
      title: "Source Current Project",
      type: "reference",
      to: [{ type: "project" }],
      description:
        "Optional link back to the public fundraising/current project this archive record came from.",
    }),
    defineField({
      name: "demographic",
    ...
    defineField({
      ...
      title: "Year",
      type: "number",
      validation: (r) => r.required().min(2021).max(2030),
    }),
    defineField({
      name: "status",
      title: "Status",
